...
def financials_from(string: str) -> Financials:
    ...
    result.last_maintenance = _get_last_maintenance(tokens[5], current_date)

    return result

def _get_berth_expiry(token: str, current_date: ImperialDate) -> ImperialDate:
    """Extract berth expiry date from string data."""
    berth_expiry = imperial_date_from(token)
...
def _get_salary_paid(token: str, current_date: ImperialDate) -> ImperialDate:
    """Extract salary paid date from string data."""
    salary_paid = imperial_date_from(token)
...
def _get_last_maintenance(token: str, current_date: ImperialDate) -> ImperialDate:
    """Extract last maintenance date from string data."""
    last_maintenance = imperial_date_from(token)
